...
SIMILARITY_THRESHOLD = float(os.getenv("SIMILARITY_THRESHOLD", 0.55)) # Ensure threshold is correctly set
        ...
        for idx, filename in enumerate(os.listdir(folder_path)):
            if filename.endswith(".txt"):
                ...
                with open(file_path, "r") as file:
                    ...
                    logging.info(f"Loaded document {filename} with content preview: {content[:100]}...")
...
# Initialize the vector store to use FAISS with cosine similarity
def init_vector_store():
    if loaded_faiss_index and loaded_metadata and loaded_docstore:
        ...
        index = faiss.IndexFlatIP(EMBEDDING_DIM) # Using IndexFlatIP for cosine similarity
        ...
        # Normalize and convert documents to vectors, then add to the vector store
        for doc in documents:
            normalized_doc = normalize_text(doc["content"])
            vectors = embeddings.embed_query(normalized_doc)
            norm = np.linalg.norm(vectors)
            vectors = vectors / norm # Normalize the vectors for cosine similarity
            logging.info(f"Processed document {doc['filename']} for embedding with vector: {vectors[:5]}...")
            ...
            logging.info(f"Added document {doc_id} to vector store with normalized content. Content: {normalized_doc[:200]}...")
...
# Search with scores using cosine similarity
def similarity_search_with_score(query, k=100):
    # Embed the query
    query_vector = np.array(embeddings.embed_query(query)).reshape(1, -1)
    norm = np.linalg.norm(query_vector)
    query_vector = query_vector / norm # Normalize the query vector for cosine similarity
    ...
    for i, score in zip(I[0], D[0]):
        if i != -1:  # Ensure valid index
            try:
                ...
                logging.info(f"Matched document {doc.metadata['id']} with score {score} and content preview: {doc.page_content[:200]}...")  # Show first 200 characters of content for brevity
    ...
    for res in results:
        logging.info(f"Document ID: {res[0].metadata['id']}, Score: {res[1]}, Content: {res[0].page_content}")
...
if __name__ == "__main__":
    test_query = "Rourke was faced with a choice: to take the treasures and exploit the harbor or to protect this untouched paradise"
    logging.info(f"Testing similarity search with query: {test_query}")
    results = similarity_search_with_score(test_query, k=TOP_SIMILARITY_RESULTS)
    for doc, score in results:
        logging.info(f"Document ID: {doc.metadata['id']}, Score: {score}, Content: {doc.page_content[:200]}...")